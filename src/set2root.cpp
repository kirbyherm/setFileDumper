...
/** Read a pixie16 .set file and store the integer values in a vector
  ...
  */
int readSetFile(std::vector<parameter> &params, const char *set_filename_, const size_t &len_=1280, const int numofmods_=0){
	if(params.empty()){ 
return -1; }
...
void help(char * prog_name_){
	...
	std::cout << "    --num [number] | Specify the number of modules used in the .set file(default=0)\n";
	std::cout << "  Hint: var file likely in /usr/opt/ddas/firmware/[version]/dsp/[module].var\n";
}
...
int main(int argc, char *argv[]){
	...
	while(index < argc){
		...
		else if(strcmp(argv[index], "--num") == 0){
			if(index + 1 >= argc){
				std::cout << " Error! Missing required argument to '--num'!\n";
				help(argv[0]);
				return 1;
			}
			nummod = atoi(argv[++index]);
			std::cout << " Outputting " << nummod << " modules. \n";
		}
		else{ 
	...
	// Read the .set file.
    if(nummod == 0){
        std::cout << "Number of modules not set." << std::endl;
	    help(argv[0]);
    }
	int readEntries = readSetFile(params, setFilename.c_str(), 1280 ,nummod);
	if(readEntries > 0){
